...
const Modal = ({ isOpen, attributes, setAttributes }) => {
    ...
    return (
        <div style={{ width: modalWidth + 'px', borderRadius: modalBorderRadius + 'px', textAlign: modalTextAlign }} id="modal-content">
            <RichText tagName={modalHeadingTagName} placeholder="Modal Title" value={name} onChange={(newName) => {
                setAttributes({ name: newName });
            }} />
            <RichText tagName='h5' placeholder="Enter text" value={age} onChange={(newAge) => {
                setAttributes({ age: newAge });
            }} />
            <RichText tagName='h5' placeholder="Enter descriptions" value={email} onChange={(newEmail) => {
