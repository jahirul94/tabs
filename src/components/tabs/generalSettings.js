...
const GeneralSettings = ( { tabTags, attributes, setAttributes } ) => {
	...
	return (
		<div>
			...
			<MenuGroup label={ __( 'Tab Direction', 'demo-tabs' ) }>
				<ButtonGroup
				...
				>
					<Button
						...
						onClick={ () =>
							setAttributes( { buttonAlignment: 'row' } )
						}
					>
						Row <Dashicon icon="arrow-right" />
					</Button>

					<Button
						className="button-tb-alignment"
						onClick={ () =>
							setAttributes( { buttonAlignment: 'column' } )
						}
					>
						Column <Dashicon icon="arrow-down" />
					</Button>
				</ButtonGroup>
			</MenuGroup>
			<Divider />
				...
				>
					{ [ 'left', 'center', 'right' ].map( ( align, i ) => (
						<Button
							...
							title={ align }
							onClick={ () =>
								setAttributes( { tabTextAlignment: align } )
							}
						>
							<Dashicon icon={ `align-${ align }` } />
						</Button>
					) ) }
				</ButtonGroup>
			</MenuGroup>
			<Divider />
			<MenuGroup label={ __( 'Tab Width Controller', 'demo-tabs' ) }>
				<RangeControl
					value={ tabWidth }
					min={ 250 }
					max={ 800 }
					onChange={ ( w ) => setAttributes( { tabWidth: w } ) }
				/>
			</MenuGroup>
		</div>
	);
