...
function Edit({ attributes, setAttributes }) {
	...
	return (
		<div {...useBlockProps()}>
			<ModalButton setIsModalOpen={setIsModalOpen} isModalOpen={isModalOpen} setAttributes={setAttributes}></ModalButton>
			<InspectorControls>
				<PanelBody title={__('Modal Settings Options', 'demo-tabs')}>
					...
					{buttonType === 'textBox' && <>
						...
						<RichText onChange={(e) => setAttributes({ openButton: e })} tagName='h5' value={openButton} placeholder='Modal Open Button' />
						<RichText onChange={(e) => setAttributes({ closeButton: e })} tagName='h5' value={closeButton}
					...
					{buttonType === 'icon' && <>
						...
						<RichText onChange={(e) => setAttributes({ openIcon: e })} tagName='h5' value={openIcon} placeholder='Enter Icon Name' />

						<RichText onChange={(e) => setAttributes({ closeIcon: e })} tagName='h5' value={closeIcon}
					...
					<Tooltip text="button-groups information">
						...
						<ButtonGroup>
							<Button onClick={() => setAttributes({ buttonLayout: 'primary' })} variant="primary">Primary</Button>
							<span> </span>
							<Button style={{ marginBottom: '5px' }} onClick={() => setAttributes({ buttonLayout: 'secondary' })} variant="secondary">Secondary</Button>

							<Button variant="secondary" onClick={() => setAttributes({ buttonLayout: 'custom' })}>Customize <span> </span><Dashicon icon='admin-customizer' /></Button>
						</ButtonGroup>
					</Tooltip>
					<Divider />
					...
					{buttonLayout === 'custom' && <Tooltip text="Button Color information">
						...
						<ColorPalette value={buttonBgColor} onChange={(e) => setAttributes({ buttonBgColor: e })} />

						<h3>Font Size</h3>
						<RangeControl min={14} max={25} onChange={(c) => setAttributes({ buttonFontSize: c })} value={buttonFontSize} /><Divider />
					</Tooltip>}
					...
					<MenuGroup label="Modal Content Settings">
						...
						<RangeControl min={0} max={30} onChange={(c) => setAttributes({ modalBorderRadius: c })} value={modalBorderRadius} />
						...
						<ButtonGroup style={{ display: 'flex', justifyContent: 'center' }}>
							<Button onClick={() => setAttributes({ modalTextAlign: 'left' })}><Dashicon icon='align-left' /></Button>
							<Button onClick={() => setAttributes({ modalTextAlign: 'center' })}><Dashicon icon='align-center' /></Button>
							<Button onClick={() => setAttributes({ modalTextAlign: 'right' })}><Dashicon icon='align-right' /></Button>
						</ButtonGroup>
